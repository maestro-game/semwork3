...
public class DtoRepositoryImpl implements DtoRepository {
    ...
    private final RowMapper<PreviewSourceDto> rowMapper = (rs, rowNum) -> new PreviewSourceDto(rs.getLong("id"),
            rs.getString("name"),
    ...
    public List<PreviewSourceDto> findAllPreviewSourceDtoByMember(Long id) {
        return jdbcTemplate.query("select s.id as id, s.name as name, m.text as lastMessageShortText, m.created as lastMessageTimestamp " +
                ...
                "         join user_source as u on u.user_id = ? and u.source_id = s.id " +
                "         join (select distinct on (m.source_id) m.source_id, m.text, m.created from message as m order by m.source_id, m.created desc) as m on s.id = m.source_id;", rowMapper, id);
    }
}
